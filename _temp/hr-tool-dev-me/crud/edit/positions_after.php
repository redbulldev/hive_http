...
{
    DB::table('positions_requester')->where(['position_id' => $id, 'status' => 0])->delete();
...
{
    DB::table('level_positions')->where(['position_id' => $id])->delete();
    ...
    foreach ($data->levels as $level_id) 
    {
        ...
        if (isset($data->parent_id)) 
        {
            ...
            if (!$check) 
            {
                DB::table('level_positions')->insert($pradd2);
            }
        }
    }
}
...
{
    DB::table('positions_requester')->where(['position_id' => $id, 'status' => 1])->delete();
