        ...
        foreach ($uploadedFiles as $file) {
            ...
            foreach ($list as $index => $row) {
                if ($index > 1) {
                    ...
                    $item['datemodified'] = date('Y-m-d H:i:s');
                    $item['step'] = 0;
                    $item['status'] = 2;
                    $item['isdelete'] = 0;
                    $newdata = removecolumn('cv', $item);
                    $id = DB::table('cv')->insertGetId($newdata);
                    DB::table('cv_history')->insert(['author_id'=>$user->username,'cv_id'=>$id,'description'=>'CV được import từ Excel','datecreate'=>date('Y-m-d H:i:s')]);
                    if ($index == 101) break;
                } else {
                    foreach ($collection as $key => $val) {
            ...
            $results = ['status' => 'success', 'data' => $numberOk, 'error' => $listError, 'time' => date('Y-m-d H:i:s')];
        }
    }
} catch (Exception $e) {
